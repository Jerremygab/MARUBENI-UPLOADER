...
class APCreditMemo():
    def processDataFrame(self, file_name):
        ...
        apcmCodeListArr = Utilities.Utilities.apcmQueryValidator()
        apcmcode_list = apcmCodeListArr[0]
        apcmcode_linenum_list = apcmCodeListArr[1]
        ...
        # df = df.loc[df['A/C Code'].isin(['44114'])]
        df = df.loc[df['2nd Sub-Account'].str.contains('CM', na=False)]
        df = df.fillna('')
        rowcount = len(df)
        ...
        for ind in df3.index:
            ...
            # APCM VALIDATION
            # APCM CODE
            print(df3['Cont.No.'][ind])
            try:
                if(df3['A/C Code'][ind] != "44114"):
                    if(df3['Cont.No.'][ind] in apcmcode_list):
                        ...
                        # apcmcode_linenum_list.index(array1)
                        
                        # if(apcmcode_linenum_list is not None):
            ...
            self.label_apcm_total_bp_not_exist.setText("BP Does Not Exist: " + str(totalBPNotExist))
            self.label_apcm_total_item_not_exist.setText("Item Does Not Exist: " + str(totalItemNotExist))
            self.label_apcm_total_exist.setText("Doc. Already Exist: " + str(totalGrpoExist))
    ...
    def loadCsv(self, file_name):
        ...
        df3 = APCreditMemo.processDataFrame(self, file_name)
        df3.to_csv('apcm.csv', index=False)
        rowcount = len(df3)
        self.tableWidget_apcm.setColumnCount(24)
        self.tableWidget_apcm.setRowCount(rowcount)

        for i in range(rowcount):
            if(str(df3._get_value(i, 18, takeable=True)) == '1' and str(df3._get_value(i, 0, takeable=True))== "SUCCESS"):
                totalready += 1
            ...
            for j in range(24):
                ...
                self.tableWidget_apcm.setItem(i,j,QTableWidgetItem(str(df3._get_value(i, j, takeable=True))))


                if(str(df3._get_value(i, 18, takeable=True)) == '1'):
                    self.tableWidget_apcm.item(i,j).setBackground(QtGui.QColor(173, 216, 230))
                else:
                    self.tableWidget_apcm.item(i,j).setBackground(QtGui.QColor(240, 255, 255))



                if(str(df3._get_value(i, 0, takeable=True))== "FAILED"):
                    self.tableWidget_apcm.setItem(i,j,QTableWidgetItem(str(df3._get_value(i, j, takeable=True))))
                    self.tableWidget_apcm.item(i,0).setBackground(QtGui.QColor(255, 128, 128))
                    # print(str(df3._get_value(i, j, takeable=True)))
                    try:
                        if("InvalidBP" in str(df3._get_value(i, j, takeable=True))):
                            self.tableWidget_apcm.item(i,j).setBackground(QtGui.QColor(255, 165, 0))


                        if("InvalidItem" in str(df3._get_value(i, j, takeable=True))):
                            self.tableWidget_apcm.item(i,j).setBackground(QtGui.QColor(255,255,0))

                        
                        if("AlreadyPosted" in str(df3._get_value(i, j, takeable=True))):
                            self.tableWidget_apcm.item(i,j).setBackground(QtGui.QColor(150,75,0))
                    ...
                    self.tableWidget_apcm.item(i,0).setBackground(QtGui.QColor(13,152,186))
                    if("ControlAccount" in str(df3._get_value(i, j, takeable=True))):
                            self.tableWidget_apcm.item(i,j).setBackground(QtGui.QColor(13,152,186))

                else:
                    self.tableWidget_apcm.item(i,0).setBackground(QtGui.QColor(144, 238, 144))


        self.label_apcm_total_ready.setText("AP CREDIT MEMO READY TO POST: " + str(totalready))


    def postAPCreditMemo(self,file_name):
        print(file_name)
        ...
        # print(type(df3))
        df4 = df3.loc[df3['Status'] == "SUCCESS"] 
        headers = df4.loc[df4['Items'] == 1]
        

        df4.to_csv('apcm.csv', index=False)
        ...
        for ind in headers.index:
            ...
            dflines = df4.loc[df4['Cont.No.'] == headers.at[ind,'Cont.No.']] 
            documentlines = []
            control_account = ''
            if(str(dflines['A/C Code'][ind]) != "44114"):
                control_account = '44114'
            ...
            for ind2 in dflines.index:
                ...
                if(str(dflines['Material Code'][ind2]) != ''):
                    if(str(dflines['A/C Code'][ind2]) != "44114"):
                        linesdict =	{
