import sys
from PyQt5.QtWidgets import QApplication, QWidget, QMainWindow, QFileDialog, QTableWidgetItem, QMessageBox, QComboBox
...
import modules.apinvoice as APInvoice
...
class Grpo():
    def processDataFrame(self, file_name):
        ...
        for ind in df3.index:
            ...
            # GRPO CODE
            try:
                if(str(df3['A/C Code'][ind]) != "41240"):
                    if(str(df3['Doc.No.'][ind]) in grpocode_list):
            ...
            self.label_grpo_total_bp_not_exist.setText("BP Does Not Exist: " + str(totalBPNotExist))
            self.label_grpo_total_item_not_exist.setText("Item Does Not Exist: " + str(totalItemNotExist))
            self.label_grpo_total_exist.setText("Doc. Already Exist: " + str(totalGrpoExist))
    ...
    def loadCsv(self, file_name):
        ...
        df3 = Grpo.processDataFrame(self, file_name)
        rowcount = len(df3)
        self.tableWidget_grpo.setColumnCount(26)
        self.tableWidget_grpo.setRowCount(rowcount)
        for i in range(rowcount):
            ...
            for j in range(26):
                ...
                self.tableWidget_grpo.setItem(i,j,QTableWidgetItem(str(df3._get_value(i, j, takeable=True))))
                
                if(str(df3._get_value(i, 18, takeable=True)) == '1'):
                    self.tableWidget_grpo.item(i,j).setBackground(QtGui.QColor(173, 216, 230))
                else:
                    self.tableWidget_grpo.item(i,j).setBackground(QtGui.QColor(240, 255, 255))


                if(str(df3._get_value(i, 0, takeable=True))== "FAILED"):
                    self.tableWidget_grpo.item(i,0).setBackground(QtGui.QColor(255, 128, 128))
                    # print(str(df3._get_value(i, j, takeable=True)))
                    try:
                        if("InvalidBP" in str(df3._get_value(i, j, takeable=True))):
                            self.tableWidget_grpo.item(i,j).setBackground(QtGui.QColor(255, 165, 0))


                        if("InvalidItem" in str(df3._get_value(i, j, takeable=True))):
                            self.tableWidget_grpo.item(i,j).setBackground(QtGui.QColor(255,255,0))

                        
                        if("AlreadyPosted" in str(df3._get_value(i, j, takeable=True))):
                            self.tableWidget_grpo.item(i,j).setBackground(QtGui.QColor(150,75,0))
                    ...
                    self.tableWidget_grpo.item(i,0).setBackground(QtGui.QColor(13,152,186))
                    if("ControlAccount" in str(df3._get_value(i, j, takeable=True))):
                            self.tableWidget_grpo.item(i,j).setBackground(QtGui.QColor(13,152,186))
                    ...
                    self.tableWidget_grpo.item(i,0).setBackground(QtGui.QColor(144, 238, 144))


            
            self.label_grpo_total_ready.setText("GRPO READY TO POST: " + str(totalready))
    ...
    def postGRPO(self,file_name):
        ...
        for ind in headers.index:
            ...
            for ind2 in dflines.index:
                if(str(dflines['A/C Code'][ind2]) != "41240"):
                    linesdict =	{
